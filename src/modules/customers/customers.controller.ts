...
export class CustomersController {
  ...
  async findAll(@Query() queryParams: PaginationDto) {
    try {
      ...
      if (paginationDto.pagNo && paginationDto.limit) {
        const { data, total } = await this.customersService.findAllWithPagination(
          paginationDto.pagNo,
          paginationDto.limit,
          {
  ...
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async update(@Param('id') id: string, @Body() reqBody: Partial<UpdatePatientDto>) {
    try {
